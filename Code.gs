...
const FOLDER_ID      = '--------------';               // 📁 ID Root Folder Google Drive (File Upload)

function testDriveAccess() {
  // ── Langkah 1: Baca folder (scope read) ──────────────────────────────────
  var folder;
  try {
    folder = DriveApp.getFolderById(FOLDER_ID);
    Logger.log('✅ Step 1 OK — Folder ditemukan: ' + folder.getName());
  } catch(e) {
    Logger.log('❌ Step 1 GAGAL — Tidak bisa baca folder.');
    Logger.log('   Kemungkinan: FOLDER_ID salah, atau belum otorisasi DriveApp.');
    Logger.log('   → Buka https://myaccount.google.com/permissions → hapus akses app ini → jalankan fungsi ini lagi.');
  ...
  var testFile;
  try {
    var testBlob = Utilities.newBlob('BMS Drive Test - Boleh dihapus', 'text/plain',
      'BMS_DriveTest_' + new Date().getTime() + '.txt');
    testFile = folder.createFile(testBlob);
    Logger.log('✅ Step 2 OK — Write permission aktif. File test: ' + testFile.getUrl());
  } catch(e) {
    Logger.log('❌ Step 2 GAGAL — Tidak bisa buat file di folder.');
    Logger.log('   Ini adalah error "Access denied: DriveApp" yang Anda alami.');
    Logger.log('   SOLUSI WAJIB:');
    Logger.log('   1. Buka https://myaccount.google.com/permissions');
    Logger.log('   2. Cari nama project GAS Anda → Hapus Akses (Remove Access)');
    Logger.log('   3. Kembali ke editor GAS → jalankan fungsi ini lagi');
    Logger.log('   4. Ketika muncul popup otorisasi → izinkan SEMUA scope');
    Logger.log('   Error: ' + e.message);
    return 'GAGAL Step 2 (write): ' + e.message;
  }

  try {
    // Coba hapus via trash dulu
    DriveApp.getFileById(testFile.getId()).setTrashed(true);
    Logger.log('✅ Step 3 OK — File test dihapus dari Drive.');
  } catch(e) {
    // Tidak bisa hapus otomatis — tidak kritis, hapus manual di Drive jika perlu
    Logger.log('ℹ️  Step 3 INFO — File test tidak bisa dihapus otomatis (tidak masalah).');
    Logger.log('   Hapus manual jika perlu: nama file dimulai dengan "BMS_DriveTest_"');
  }

  
  try {
    PropertiesService.getScriptProperties().setProperty('bms_drive_test', 'ok_' + new Date().getTime());
    PropertiesService.getScriptProperties().deleteProperty('bms_drive_test');
    Logger.log('✅ Step 4 OK — PropertiesService berfungsi.');
  } catch(e) {
    Logger.log('❌ Step 4 GAGAL — PropertiesService error: ' + e.message);
    return 'GAGAL Step 4 (PropertiesService): ' + e.message;
  }

  Logger.log('');
  Logger.log('🎉 SEMUA TEST LULUS! Silakan lanjutkan ke Deploy.');
...
function doGet() {
  return HtmlService.createHtmlOutputFromFile('index')
    .setTitle('BMS Purchasing Dashboard')
...
function login(username, password) {
  try {
    ...
    // Columns: username(0), password(1), name(2), role(3), lastLogin(4), created(5)
...
// ─── Account: Simpan URL foto profil ke sheet Users (kolom 7) ─────────────────
// Struktur sheet Users: col1=username, col2=password, col3=name, col4=role,
//                       col5=lastLogin, col6=created, col7=photoUrl
...
function saveRecord(sheetName, rowData, rowIndex) {
  try {
    ...
    if (!sheet) {
      ...
      Logger.log('Sheet "' + sheetName + '" dibuat otomatis dengan ' + newHeaders.length + ' kolom.');
    }
    ...
    const headers = sheet.getRange(1, 1, 1, currentLastCol).getValues()[0].map(function(h){ return String(h).trim(); });
    ...
    if (missingKeys.length > 0) {
      ...
      Logger.log('Kolom baru ditambahkan ke sheet "' + sheetName + '": ' + missingKeys.join(', '));
    }
...
// Buat folder bertingkat dari array path, misal ['Purchase Order','Nama Item']
...
// ─── Mapping menu → nama folder di Drive ──────────────────────────────────────
var MENU_FOLDER_MAP = {
...
// b64        : Base64 string isi file
// fileName   : nama file asli
// mimeType   : MIME type
// subFolder  : nama item/barang (level 2)
// menuSection: key section misal 'purchase-orders' (level 1, opsional)
function uploadFileToDrive(b64, fileName, mimeType, subFolder, menuSection) {
  try {
    ...
    if (!FOLDER_ID || FOLDER_ID === 'FOLDER_ID')
      return { success: false, error: 'FOLDER_ID belum dikonfigurasi di Kode.gs.' };

    // Verifikasi akses folder sebelum upload
    var rootFolder;
    try {
      rootFolder = DriveApp.getFolderById(FOLDER_ID);
    } catch(folderErr) {
      return { success: false, error: 'Gagal akses folder Drive. Pastikan FOLDER_ID benar dan jalankan testDriveAccess() lalu re-deploy. Detail: ' + folderErr.message };
    }

    // Bangun path folder: [Nama Menu] / [Nama Item]
    var pathParts = [];
    ...
    var targetFolder = pathParts.length > 0
      ? getOrCreateFolderPath_(rootFolder, pathParts)
      : rootFolder;
    ...
    // Set sharing publik (opsional — tidak crash jika ditolak admin)
    try {
      file.setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW);
    } catch(shareErr) {
      Logger.log('⚠️  setSharing tidak diizinkan (mungkin dibatasi admin): ' + shareErr.message);
    }
    ...
    return {
      ...
      folderName: subFolder || menuLabel || null
    };
  } catch(e) {
...
function initChunkedUpload(fileName, mimeType, subFolder, menuSection) {
  try {
    ...
    try {
      ...
      return { success: false, error: 'Gagal akses folder Drive saat init chunked upload. Jalankan testDriveAccess() lalu re-deploy. Detail: ' + folderErr.message };
    }
    ...
    // Set sharing (opsional — tidak crash jika dibatasi admin)
    try {
      placeholder.setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW);
    } catch(shareErr) {
      Logger.log('⚠️  setSharing placeholder tidak diizinkan: ' + shareErr.message);
    }
    ...
    Logger.log('❌ initChunkedUpload error: ' + e.message);
...
function finalizeChunkedUpload(tempKey) {
  try {
    ...
    try {
      ...
      Logger.log('⚠️  setSharing file final tidak diizinkan: ' + shareErr.message);
    }

    // Hapus placeholder kosong yang dibuat saat init
    try { DriveApp.getFileById(meta.fileId).setTrashed(true); } catch(e2) {}

    // Bersihkan PropertiesService
    props.deleteProperty(tempKey);

    Logger.log('✅ Chunked upload finalized: ' + meta.fileName + ' → ' + meta.folderPath);

    return {
      success:    true,
      url:        'https://drive.google.com/file/d/' + newFile.getId() + '/view?usp=sharing',
...
// ─── Bersihkan sesi upload lama (opsional, bisa dijalankan manual) ────────────
        ...
        if (now - meta.created > twoHours) {
          for (var i = 0; i < meta.chunks; i++) {
            PropertiesService.getScriptProperties()
              .deleteProperty(k + '_c' + String(i).padStart(4,'0'));
          }
  ...
  try {
    ...
    if (npwpSheet) {
      ...
      if (lastRow > 1 && lastCol >= 2) {
        var headers = npwpSheet.getRange(1, 1, 1, lastCol).getValues()[0]
    ...
    // Sumber 2: sheet Invoice — ambil data unik Nama Perusahaan, NPWP, Alamat
    ...
    if (invSheet) {
      ...
      if (invLast > 1) {
        ...
        var seen = {};
        // Kumpulkan nama dari DATABASE NPWP supaya tidak duplikat
